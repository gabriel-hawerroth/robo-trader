...
def produzir_estrategia(dados_historicos):
    ...
    BB_estudo = ta.BBANDS(
        dados_historicos["Close"], timeperiod=14, nbdevup=2.0, nbdevdn=2.0, matype=0
    )
    ...
    dados_estrategia["Sinal_BB"] = np.where(
        dados_estrategia["Close"] < dados_estrategia["BB_Inferior"],
        1,
        np.where(dados_estrategia["Close"] > dados_estrategia["BB_Superior"], -1, 0),
    )
    dados_estrategia["Sinal_IFR"] = np.where(dados_estrategia["IFR"] > 15, 1, 0)
    dados_estrategia["Decisao"] = np.where(
        (dados_estrategia["Sinal_BB"] == 1) & (dados_estrategia["Sinal_IFR"] == 1),
        "Compra",
        np.where(
            (dados_estrategia["Sinal_BB"] == -1) & (dados_estrategia["Sinal_IFR"] == 1),
            "Venda",
...
while True:
    ...
    intervalo = (
        mt5.TIMEFRAME_M15
    )  # Constante utilizada pelo MetaTrader5 para intervalo de 15 minutos.
    inicio = dt.datetime(
        2021, 1, 4
    )  # Data inicial. Para fins de exemplo, utilizamos dia 04/01/2021.
    ...
    if (
        ativo
        in ativo
        in [ativos_disponiveis[n].name for n in range(len(ativos_disponiveis))]
    ):
        mt5.symbol_select(ativo, True)

    else:
        print(
            "Ativo selecionado inválido. Verifique se o mesmo está disponível na plataforma do MetaTrader5."
        )
    ...
    horario_atual = dt.datetime.strftime(
        horario_atual, "%Y-%m-%d %H:%M"
    )  # Horário atual em formato de texto.
    volume = 100.0  # Volume a ser comprado/vendido de determinado ativo. Padrão definido em 100 = 1 lote de ações.

    if (dados_estrategia["Decisao"].iloc[-1] == "Compra") and (
        ativo not in ativos_comprados
    ):  # Se decisão é compra e o ativo ainda não foi comprado.
    ...
    elif (dados_estrategia["Decisao"].iloc[-1] == "Venda") and (
        ativo in ativos_comprados
    ):  # Se decisão é venda e o ativo já esteja comprado.
